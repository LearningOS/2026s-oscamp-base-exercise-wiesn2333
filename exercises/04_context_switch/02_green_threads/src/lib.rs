...
impl Scheduler {
    ...
    pub fn spawn(&mut self, entry: extern "C" fn()) {
        ...
        let stack_top = stack.as_ptr() as *const u8 as usize + STACK_SIZE;
        let ctx = TaskContext {
            sp: stack_top as u64,
    ...
    fn schedule_next(&mut self) {
        ...
        let next = {
            ...
            while self.threads[next].state == ThreadState::Finished {
        ...
        self.current = next;
        unsafe {
            let current_ctx: *mut TaskContext = self.threads[current].ctx.as_mut_ptr();
            let next_ctx = self.threads[next].ctx.as_ptr();
            switch_context(&mut *current_ctx, &*next_ctx);
        }
    }
}
